...
import copla.constraints.meta.stn.core.StnWithStructurals._

class StnWithStructurals(
    var nonRigidIndexes: mutable.Map[Timepoint, Int],
  ...
  override def clone(): StnWithStructurals = new StnWithStructurals(
    nonRigidIndexes.clone(),
    timepointByIndex.clone(),
    dist.clone(),
    rigidRelations.clone(),
    contingentLinks.clone(),
    optStart,
    optEnd,
    originalEdges,
    consistent,
    executed.clone(),
    watchedVarsByIndex.map(kv => (kv._1, kv._2.clone()))
  )
  ...
  def timepoints: List[Timepoint] = _timepoints.toList
  ...
  private def isKnown(tp: Timepoint) =
    nonRigidIndexes.contains(tp) || rigidRelations.isAnchored(tp)
  ...
  def forceExecutionTime(tp: Timepoint, time: Int): Unit = {
    def neighborhood(tp: Timepoint) =
      if (rigidRelations.isAnchor(tp))
        ...
        rigidRelations.getTimepointsAnchoredTo(rigidRelations.anchorOf(tp)).toSet + rigidRelations
          .anchorOf(tp) - tp
  ...
  final def canBeStrictlyBefore(u: Timepoint, v: Timepoint): Boolean =
    isConstraintPossible(v, u, -1)

  final def isDelayPossible(from: Timepoint, to: Timepoint, delay: Int) =
  ...
  def concurrent(tp1: Timepoint, tp2: Timepoint) =
    rigidAwareDist(tp1, tp2) == rigidAwareDist(tp2, tp1)
  ...
  private def beforeOrConcurrent(first: Timepoint, second: Timepoint) =
    rigidAwareDist(second, first) <= NIL
  private def strictlyBefore(first: Timepoint, second: Timepoint) =
    rigidAwareDist(second, first) < NIL
  private def between(tp: Timepoint, min: Timepoint, max: Timepoint) =
    beforeOrConcurrent(min, tp) && beforeOrConcurrent(tp, max)
  private def strictlyBetween(tp: Timepoint, min: Timepoint, max: Timepoint) =
    strictlyBefore(min, tp) && strictlyBefore(tp, max)
  ...
  override def distancesUpdated(edges: Seq[(Int, Int)]): Unit = {
    ...
    val watchesToNotify
      : Set[(Timepoint, Timepoint)] = // extract watches while making a defensive copy
      edges.flatMap {
        case (a: Int, b: Int) => watchedVarsByIndex.getOrElse(asWatchKey(a, b), Nil)
      }.toSet

    // check if the network became inconsistent
    for ((a, b) <- edges) {
      if (dist.getDistance(a, b) + dist.getDistance(b, a) < 0) {
        assert4(!consistentWithBellmanFord(),
                "BellmanFord and APSP gave different results regarding consistency")
        consistent = false
    ...
    assert3(watchesToNotify.forall(p => isWatched(p._1, p._2)),
            "An edge became unwatched when processing the rigid relations")
    assert1(watchesToNotify.isEmpty || distanceChangeListener.nonEmpty,
            "Changes to notify but no recorded listener")
      ...
      val (anchored, anchor) =
        if (rigidRelations.isAnchored(tpA)) (tpA, tpB)
          ...
          throw new RuntimeException(
            "No timepoint is considered as anchored after recording a new rigid relation")
  ...
  def addWatchedDistance(tp1: Timepoint, tp2: Timepoint) {
    ...
    assert1(distanceChangeListener.nonEmpty,
            "Added a watch but no distance change listener plugged in.")
    assert2(isWatched(tp1, tp2))
    assert3(watchedVarsByIndex.toSeq.forall {
      case (k, v) => v.forall(p => k == watchKey(p._1, p._2))
    })
  }
  ...
  def isConsistent: Boolean = {
    ...
    contingentLinks.forall(l =>
      isDelayPossible(l.src, l.dst, l.min) && isConstraintPossible(l.src, l.dst, l.max)))
  }
  ...
  def getConstraintsWithoutStructurals: List[TemporalConstraint] = {
    ...
    def structuralNeighborhood(neighborhood: Set[Timepoint],
                               nextNeighbors: Set[Timepoint]): Set[Timepoint] = {
      ...
      val directNeighbors =
        originalEdges // should include neighborhood of other (non-rigid?) structurals
          .filter(e => e.from == tp || e.to == tp)
          .flatMap(e => e.from :: e.to :: Nil)
          .filter(_.isStructural)
          .toSet
      return structuralNeighborhood(neighborhood + tp,
                                    nextNeighbors ++ (directNeighbors -- neighborhood) - tp)
    }
    ...
    for (tp <- timepoints if tp.isStructural && rigidRelations.isAnchor(tp)) {
    ...
    for (tp <- timepoints if !tp.isStructural && rigidRelations.isAnchored(tp) && !rigidRelations
           .anchorOf(tp)
           .isStructural) {
