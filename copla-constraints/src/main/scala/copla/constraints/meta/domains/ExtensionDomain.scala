...
class ExtensionDomain(val numVariables: Int) {
  ...
  def addTuple(tuple: Seq[Int]) {
    assert1(
      !locked,
      "Trying to add a tuple to and extension domain already locked (i.e. that has been used for propagation)")
  ...
  def restrictedDomains(originalDomains: Seq[Domain]): Seq[Domain] = {
    ...
    val unaryReducedDomains =
      individualDomains.zip(originalDomains).map { case (x, y) => x.intersection(y) }
