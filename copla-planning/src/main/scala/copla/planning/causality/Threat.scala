...
import copla.constraints.meta.constraints.{Constraint, DisjunctiveConstraint, Tautology}
import copla.constraints.meta.events.Event
import copla.constraints.meta.variables.IVar
import copla.planning.structures.{Change, Holds, PStruct}

object Threat {

  def apply(c1: Change, c2: Change) : Constraint = {
    val disjuncts = List(c1.fluent =!= c2.fluent, c1.persists.end <= c2.changing.start, c2.persists.end <= c1.changing.start)
    new Threat(disjuncts, c1, c2)
  }

  def apply(c: Change, h: Holds) : Constraint = {
    val disjuncts = List(
      c.fluent =!= h.fluent, // only applicable if on the same state variable
  ...
  def apply(h1: Holds, h2: Holds) : Constraint = {
    val disjuncts = List(h1.fluent =!= h2.fluent, h1.value === h2.value, h1.persists.end < h2.persists.start, h2.persists.end <= h1.persists.start)
